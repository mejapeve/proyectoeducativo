...
@if(isset($moment))
    ...
    <div class="fs--2 font-weight-semi-bold">
        ...
        <a class="cursor-pointer d-flex" href="{{route('student.show_moment_section',['empresa'=>auth('afiliadoempresa')->user()->company_name(), 'sequence_id' => $sequence_id, 'moment_id' => $moment->id, 'section_id' => ($index+1),'account_service_id'=>$account_service_id])}}">
            <img src="{{asset('/images/icons/preguntaCentral.png')}}" height= "auto" width="45px">
            <h6 class="ml-3 mb-auto mt-auto">Pregunta central:<small>&nbsp;{{$section['title']}}</small></h6>
        </a>
        @endif
        @if($section['section']['type'] == 2)
        <a class="cursor-pointer d-flex" href="{{route('student.show_moment_section',['empresa'=>auth('afiliadoempresa')->user()->company_name(), 'sequence_id' => $sequence_id, 'moment_id' => $moment->id, 'section_id' => ($index+1),'account_service_id'=>$account_service_id])}}">
            <img src="{{asset('/images/icons/cienciaCotidiana.png')}}" height= "auto" width="45px">
            <h6 class="ml-3 mb-auto mt-auto">Ciencia en contexto:<small>&nbsp;{{$section['title']}}</small></h6>
        </a>
        @endif
        @if($section['section']['type'] == 3)
        <a class="cursor-pointer d-flex" href="{{route('student.show_moment_section',['empresa'=>auth('afiliadoempresa')->user()->company_name(), 'sequence_id' => $sequence_id, 'moment_id' => $moment->id, 'section_id' => ($index+1),'account_service_id'=>$account_service_id])}}">
            <img src="{{asset('/images/icons/iconoExperiencia.png')}}" height= "auto" width="45px">
            <h6 class="ml-3 mb-auto mt-auto">Experiencia científica:<small>&nbsp;{{$section['title']}}</small></h6>
        </a>
        @endif
        @if($section['section']['type'] == 4)
        <a class="cursor-pointer d-flex" href="{{route('student.show_moment_section',['empresa'=>auth('afiliadoempresa')->user()->company_name(), 'sequence_id' => $sequence_id, 'moment_id' => $moment->id, 'section_id' => ($index+1),'account_service_id'=>$account_service_id])}}">
            <img src="{{asset('/images/icons/masConexiones.png')}}" height= "auto" width="45px">
            <h6 class="ml-3 mb-auto mt-auto"> + Conexioness:<small>&nbsp;{{$section['title']}}</small></h6>
        </a>
        @endif
        @endforeach
        @endif
